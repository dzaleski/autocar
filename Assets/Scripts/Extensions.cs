...
namespace Assets.Scripts.Extensions
{
    public static class Extensions
    {
        ...
        public static void FocusOn(this Camera camera, float horizontalSize, Vector3 lookAt, float marginPercentage)
        {
            float maxExtent = horizontalSize;
            float minDistance = (maxExtent * marginPercentage) / Mathf.Sin(Mathf.Deg2Rad * camera.fieldOfView / 2f);
            camera.transform.position = lookAt + Vector3.up * minDistance;
